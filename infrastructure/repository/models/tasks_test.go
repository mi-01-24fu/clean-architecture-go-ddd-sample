...
var (
	taskDBTypes = map[string]string{`ID`: `integer`, `UsersID`: `integer`, `Title`: `character varying`, `Content`: `text`, `Date`: `date`, `Status`: `character varying`, `Priority`: `character varying`, `CreatedAt`: `timestamp without time zone`, `UpdatedAt`: `timestamp without time zone`}
	_           = bytes.MinRead
)
